...
actor UpdateService {
    ...
    ///   than answered from a fetch made inside the last few minutes.
    ...
    func restart() async -> (accepted: Bool, status: UpdateStatus) {
        ...
        begin(.restart, automatic: false, target: OmpVersion.onDisk()?.version)
        beginRestart()
        return (true, await status(refreshing: false))
    }
    ...
    private func concludeJob() {
        ...
        let (outcome, reason) = UpdateJob.conclusion(
            of: current, runningBuiltAt: OmpVersion.running?.builtAt ?? Self.executableModified(),
    ...
    private func follow(installerPhase phase: String?) {
        ...
        if before == .download, current.kind == .update, let source {
            current.target = OmpVersion.describe(source: source)
        }
        job = current
        writeJob()
    }
    ...
    private func runningVersion() -> String? {
        OmpVersion.running?.version ?? source.map { OmpVersion.describe(source: $0) }
    }
    ...
    private func considerUpdating() async {
        ...
        if current.restartRequired, current.canRestart {
            if job?.isFinished != false {
                begin(.restart, automatic: true, target: OmpVersion.onDisk()?.version)
            }
            beginRestart()
            return
        }
    ...
    /// A phase written from here keeps the installer's pid while the installer could still be
    /// running — but a new job inherits nothing: the pid on disk after a failed build is that
    /// build's installer, dead, and carried into the next job it would have that job judged failed
